...
@interface CCModelProperty : NSObject
...
@implementation CCModelProperty
...
- (NSString *)description {
    ...
    for (CCModelProperty *property in [self getPropertys]) {
...
- (void)setup {
    ...
    NSDictionary *mapper = [self CCKeyMapper];
 ...
 */
- (void)assginObjectProperty:(CCModelProperty *)property withJsonValue:(id)jsonValue {
...
/**
 ...
 @return 解析后的数据
 */
- (id)assginToArrayWithProperty:(CCModelProperty *)property value:(id)value {
...
/**
 ...
 @param jsonValue 对应json中的数据
 */
- (void)assginProperty:(CCModelProperty *)property withJsonValue:(id)jsonValue {
...
- (BOOL)checkPropertyType:(CCModelProperty *)property value:(id)value {
...
- (void)inspectProperties {
    ...
    for (unsigned int i = 0; i < propertyCount; i++ ) {
        CCModelProperty *p = [[CCModelProperty alloc] init];
        ...
        if ([scanner scanString:@"@\"" intoString: &propertyType]) {
            ...
            while ([scanner scanString:@"<" intoString:NULL]) {
                ...
                if ([protocolName isEqualToString:@"CCIgnore"]) {
                    p = nil;
                } else if ([protocolName isEqualToString:@"CCNilWhenNull"]) {
                    p.isAssignNilWhenNull = YES;
                } else if ([protocolName isEqualToString:@"CCCamelCase"]) {
                    p.isCamelCase = YES;
                } else {
...
/**
 获取包含属性信息的数组
 */
- (NSArray<CCModelProperty *> *)getPropertys {
...
@implementation CCModel (CCModelExtensionMethods)
...
- (NSDictionary *)toDictionary {
    ...
    for (CCModelProperty *property in [self getPropertys]) {
        if ([self respondsToSelector:property.getterMethod]) {
            ...
            } else if ([value isKindOfClass:CCModel.class]) {
                [tempDict setObject:[(CCModel *)value toDictionary] forKey:property.name];
            } else {
                if ([property.type isEqualToString:@"i"]) { // int类型
...
- (instancetype)initWithDictionarys:(NSDictionary *)dictionary, ... NS_REQUIRES_NIL_TERMINATION {
    ...
    CCModelCombinationDict(completeDict);
    self = [self init];
