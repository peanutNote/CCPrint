//
//  CMPrintTaskNormalLabel.m
//  QYBluetoothManager
//
//  Created by qianye on 2017/7/19.
...
@implementation CMPrintTaskNormalLabel
...
- (void)printWithField:(CCPrintField *)printField {
    ...
    if (![printerManager startPrintWithTaskModel:CMPrintTaskModelNormalLabel]) {
        return;
    }
    ...
    [printerManager addText:[NSString stringWithFormat:@"%@-%@ %@件", printField.srcCity.content, printField.desCity.content, printField.goodsNumber.content] attributeBlock:^(CCPrint *print) {
        [print cc_multiplexAttributes:^(CCAttributeMaker *maker) {
            maker.position.value(CGPointMake(xStart, yCursor));
        }];
        [print cc_aloneAttributes:^(CCAttributeMaker *maker) {
    ...
    [printerManager addText:[NSString stringWithFormat:@"%@：%@", printField.srcCity.content, printField.srcTel.content] attributeBlock:^(CCPrint *print) {
        [print cc_multiplexAttributes:^(CCAttributeMaker *maker) {
            maker.position.value(CGPointMake(xStart, yCursor));
        }];
    }];
    yCursor += text_common_height * 3 + text_padding;
    [printerManager addText:[NSString stringWithFormat:@"%@：%@", printField.desCity.content, printField.desTel.content] attributeBlock:^(CCPrint *print) {
        [print cc_multiplexAttributes:^(CCAttributeMaker *maker) {
            maker.position.value(CGPointMake(xStart, yCursor));
        }];
    }];
