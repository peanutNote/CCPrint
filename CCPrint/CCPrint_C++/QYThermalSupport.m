...
+ (UIColor *)getImageMainColor:(UIImage*)image {
    ...
    if (bitmapData) {
        ...
        uint32_t greed = 0;
        uint32_t blue = 0;
        for(int y = 0; (y + 24) < height; y += 24) {
            for (int x = 0; x < width; x++) {
                for (int temp_y = 0 ; temp_y < 24; ++temp_y)
                {
                    uint8_t *rgbaPixel = (uint8_t *) &bitmapData[(y+temp_y) * width + x];
                    red += rgbaPixel[RED];
                    greed += rgbaPixel[GREEN];
                    blue += rgbaPixel[BLUE];
                }
            }
        }
        
        uint32_t mainRed  = red / (width * height);
        uint32_t mainGreed  = greed / (width * height);
        uint32_t mainBlue  = blue / (width * height);
        return [UIColor colorWithRed:mainRed / 255.0 green:mainGreed / 255.0 blue:mainBlue / 255.0 alpha:1.0];
    }
    return nil;
}
